...
tm1 <- c(as.POSIXct("2024-06-01"), as.POSIXct("2024-07-15"))  #15/06 - 15/07
...
# 🔹 Dossier contenant tes fichiers SGATSummary
wd <- "C:/Users/pdoniolvalcroze/Desktop/Admin_MNHN/LUSSVENAM_Short_Com/Data_Code_LUSSVENAM/Light_data_analysis_LUSSVENAM/data/Results/LusSve"


# 🔹 Trouver tous les fichiers qui correspondent au pattern
files <- list.files(wd, pattern = "_GeoLight_crds\\.csv$", full.names = TRUE)

if (length(files) == 0) stop("Aucun fichier _GeoLight_crds.csv trouvé.")
...
# Vérification du format de la colonne Time
all_data$Time <- as.POSIXct(all_data$Time, tz = "UTC")

# 🔹 Créer des colonnes mois/jour
all_data <- all_data %>%
  mutate(month = month(Time),
         day = day(Time))

# 🔹 Filtrer entre 15 novembre et 15 février
winter_data <- all_data %>%
  filter(
    (month == 11 & day >= 15) |  # 15–30 nov
      (month == 12) |              # déc entier
      (month == 1)  |              # jan entier
      (month == 2 & day <= 15)     # 1–15 fév
  )

# 🔹 Calculer moyennes et écart-types par fichier
...
# Limites de la carte (Europe de l’Ouest)
xlim <- c(-15, 10)
ylim <- c(34, 50)
...
# Remplacer la 4ème couleur (#FF7F00) par du violet foncé (#6A3D9A)
my_colors <- c("#E41A1C", "#377EB8", "#4DAF4A", "gray20", "blue")
...
ggplot() +
  ...
  # Densité spatiale des positions
...
output_file <- "GeoLight_WinterDensityPlot_5point.tiff"

# Sauvegarder le plot
ggsave(filename = output_file,
       plot = last_plot(),     # ou mettre ton objet ggplot si assigné à une variable
       device = "tiff",
       width = 12,             # largeur en pouces
       height = 8,             # hauteur en pouces
       units = "in",
       dpi = 600,              # haute résolution pour publication
